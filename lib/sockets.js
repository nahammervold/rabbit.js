...
PubSocket.prototype.connect = function(destination, callback) {
  ...
  if (this.options.noCreate) {
    ...
    ch.assertExchange(destination,
                      ...
                      {durable: this.options.persistent || false, autoDelete: false})
      .then(function() {
        self.pubs.push(destination);
      }, function(err){
        console.log(err)
...
function SubSocket(channel, opts) {
  ...
  var setup = channel.then(function(ch) {
     // if queueName is present on ops then set the name to be passed in assertQueue
     var queueName = opts ? opts.queueName || '' : '';
     var exclusive = opts.persistent;
     ...
     var queueOptions = {
      ...
      durable: opts.persistent || false
     }
     return ch.assertQueue(queueName, queueOptions).then(function(ok) {
      ...
      }, {noAck:true, exclusive:exclusive})
